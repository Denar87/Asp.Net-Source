...
namespace ERPSSL.INV
{
    public partial class DepartmentType : System.Web.UI.Page
    {
        ...
        DepartmentTypeBLL objDept_BLL = new DepartmentTypeBLL();
        ...
        private void GetResionForDepartment()
        {
            try
            {
                ...
                var row = objDept_BLL.GetAllDepartmentResion(OCODE).ToList();
                if (row.Count > 0)
                {
        ...
        private void BridOfficeByResion(int ResionId)
        {
            try
            {
                var row = objDept_BLL.GetOfficeByResionId(ResionId).ToList();
                if (row.Count > 0)
                {
        ...
        private void GetAllDepartmentforGridview()
        {
            try
            {
                ...
                List<RDepartmentType> departments = new List<RDepartmentType>();
                departments = objDept_BLL.GetDepartmentType(OCODE).ToList();
