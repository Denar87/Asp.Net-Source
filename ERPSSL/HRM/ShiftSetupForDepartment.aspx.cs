...
{
    public partial class ShiftSetupForDepartment : System.Web.UI.Page
    {
        ...
        private void GetShiftCodeList()
        {
            try
            {
                ...
                if (row.Count > 0)
                {
                    ddlShiftName.DataSource = row.ToList();
                    ddlShiftName.DataTextField = "ShiftName";
                    ddlShiftName.DataValueField = "ShiftCode";
                    ddlShiftName.DataBind();
                    ddlShiftName.Items.Insert(0, new ListItem("------Select Shift------", "0"));
                }
            }
            catch (Exception)
            {
        ...
        protected void BtnSave_Click(object sender, EventArgs e)
        {
            try
            {
                HRM_DepartmentWiseShift objtimeSchedule = new HRM_DepartmentWiseShift();
                ...
                objtimeSchedule.ShiftCode = ddlShiftName.SelectedValue;
                objtimeSchedule.ShiftName = ddlShiftName.SelectedItem.Text;
                ...
                int ShiftCount = (from shift in _context.HRM_DepartmentWiseShift
                                  where shift.RegionId == objtimeSchedule.RegionId && shift.OfficeId == objtimeSchedule.OfficeId && shift.DepartmentId == objtimeSchedule.DepartmentId && shift.ShiftCode == objtimeSchedule.ShiftCode
                ...
                if (BtnSave.Text == "Save")
                {
                    ...
                    int result = objtimeScheduleBLL.InsertDepartmentwiseShift(objtimeSchedule);
                    if (result == 1)
                    {
        ...
        private void BindGridTimeSchedule()
        {
            ...
            {
                string OCODE = ((SessionUser)Session["SessionUser"]).OCode;
                List<HRM_DepartmentWiseShift> timeSchedule = objtimeScheduleBLL.GetAllDepartmentSchedule(OCODE).ToList();
        ...
        {
            lblMessage.Text = "";
            List<HRM_DepartmentWiseShift> objTimeShift = new List<HRM_DepartmentWiseShift>();
            ...
            try
            {
                ...
                if (lblId != null)
                {
                    ...
                    objTimeShift = objtimeScheduleBLL.GetDepartwiseScheduleById(OCODE, ShceduleId);

                    if (objTimeShift.Count > 0)
                    {
                        foreach (HRM_DepartmentWiseShift timeSchedule in objTimeShift)
                        {
                            ...
                            ddlShiftName.SelectedValue = timeSchedule.ShiftCode;
