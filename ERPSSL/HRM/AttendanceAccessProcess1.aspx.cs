...
{
    public partial class AttendanceAccessProcess1 : System.Web.UI.Page
    {
        ...
        protected void btnAttendanceProcess_Click(object sender, EventArgs e)
        {
            ...
            try
            {
               ...
               // string strquery = "SELECT eventDate,eventTime,eventCard  FROM PubEvent WHERE eventCard <>'' AND Format(eventDate, 'mm/dd/yyyy') BETWEEN  Format(#" + fromdate + "#, 'mm/dd/yyyy')  AND Format(#" + todate + "#, 'mm/dd/yyyy')";
                ...
                string strquery = "SELECT a.CHECKTIME,b.CardNo FROM CHECKINOUT a inner join USERINFO b on a.USERID=b.USERID WHERE b.CardNo <>''AND Format(a.CHECKTIME, 'mm/dd/yyyy') BETWEEN  Format(#" + fromdate + "#, 'mm/dd/yyyy')  AND Format(#" + todate + "#, 'mm/dd/yyyy')";
               // string strtime= "SELECT CHECKTIME,sn  FROM CHECKINOUT WHERE sn <>''AND Format(CHECKTIME, 'mm/dd/yyyy') BETWEEN  Format(#" + fromdate + "#, 'mm/dd/yyyy')  AND Format(#" + todate + "#, 'mm/dd/yyyy')";

               // TimeSpan? time = Convert.ToDateTime(strtime).TimeOfDay;
                // using (con = new OleDbConnection(@"PROVIDER=Microsoft.ACE.OLEDB.12.0;" + @"DATA SOURCE=F:\Data123\Attendance.accdb"))
                // Microsoft.Jet.OLEDB.4.0;
               ...
               // using (con = new OleDbConnection(@"PROVIDER= Microsoft.Jet.OLEDB.4.0;" + @"DATA SOURCE=\\192.168.0.80\F:\azim\att2000.mdb"))

                string dataPath = System.Configuration.ConfigurationSettings.AppSettings["AttendanceDataPath"];
                using (con = new OleDbConnection(@"PROVIDER= Microsoft.Jet.OLEDB.4.0;" + @"DATA SOURCE=" + dataPath)) 
                {
                    using (cmd = new OleDbCommand(strquery, con))
                    {
                        OleDbDataAdapter Da = new OleDbDataAdapter(cmd);
                ...
                foreach (DataRow aitem in ds.Tables[0].Rows)
                {
                    ...
                    _employee.E_date = aitem["CHECKTIME"].ToString();

                    _employee.EID = aitem["CardNo"].ToString();
                    _employee.GetTimeL = Convert.ToDateTime(aitem["CHECKTIME"]);
                        
                        //DateTime.ParseExact(_employee.E_date, "yyyy-MM-dd HH:mm:ss,fff",
                ...
                foreach (var aitem in employes)
                {
                    HRM_AttendanceTemp _attendace = new HRM_AttendanceTemp();
                    _attendace.EID = aitem.EID;
                    _attendace.IOTIME = aitem.GetTimeL.Value.TimeOfDay;

                    string[] date = aitem.E_date.Split(' ');
                    _attendace.AttendanceDate = Convert.ToDateTime(date[0]);
                ...
                //lblMessage.Text = "Data Process Temporarily";
        ...
        protected void btnCOnf_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var ParamempID01 = new SqlParameter("@Date_From", fromdate.Date);
                var ParamempID02 = new SqlParameter("@Date_To", todate.Date);
                string attTemp = "HRM_SaveAttendanceDataFromAttendanceTemp @Date_From,@Date_To";
                _context.ExecuteStoreCommand(attTemp, ParamempID01, ParamempID02);
                _context.CommandTimeout = 100000;
