...
namespace ERPSSL.Accounting.UI_Reporting
{
    public partial class CashFlow : System.Web.UI.Page
    {
        ...
        rpt_CashFlow_BLL objCf_BLL = new rpt_CashFlow_BLL();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                GetType = "A";
                dateFrom = String.Format("{0:MM/dd/yyyy}", Session["FinancialYear"]);
                //------------------------------------------------------
                ...
                if (!IsPostBack)
                {
                    GetCashFlow(dateFrom, dateTo, GetType);
                }
                //}
                //else
                //{
        ...
        private void GetCashFlow(string dateFrom, string dateTo, string GetType)
        {
            try
            {
                //if (GetPermission[3].ToString() == "True")
                ...
                DataTable dt = objCf_BLL.Get_AC_CashFlowStatement(ht);
                if (dt.Rows.Count > 0)
                {
        ...
        protected void dtg_list_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            try
            {
                ...
                {
                    year = Convert.ToDateTime(dateFrom).Year;
                    yearNext = Convert.ToDateTime(dateFrom).Year + 1;

                    dtg_list.HeaderRow.Cells[1].Text = "Year - " + year;
                    dtg_list.HeaderRow.Cells[2].Text = "Year - " + yearNext;
        ...
        protected void btnPrint_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                ...
                if (rdbViewAll.Checked)
                {
                    ...
                    year = Convert.ToDateTime(dateFrom).Year;
                    yearNext = Convert.ToDateTime(dateFrom).Year + 1;
                    rptCriteria = "All Records of " + year + " - " + yearNext;
                }
                else
                {
                    dateFrom = String.Format("{0:MM/dd/yyyy}", dtpTo.Text);
                ...
                PrintBalanceSheet(dateFrom, GetType, rptCriteria);
                //}
                //else
                //{
                //    ScriptManager.RegisterStartupScript(this.Page, this.Page.GetType(), "User Permission", "alert('You don't have permission to access this page!!)');", true);
        ...
        private void PrintBalanceSheet(string dateFrom, string GetType, string rptCriteria)
        {
            Hashtable ht = new Hashtable();
            ht.Add("GetType", GetType);
            ht.Add("DateFrom", dateFrom);
            ...
            DataTable dt = objCf_BLL.Rpt_Get_AC_CashFlowStatement(ht);
            if (dt.Rows.Count > 0)
            {
                Session["ReportTitle"] = "Cash Flow Statement";
                ...
                Session["rptDs"] = "ds_CashFlow";
                Session["rptDt"] = dt;
                Session["rptFile"] = "..\\UI_ReportsFile\\CashFlow.rdlc";
