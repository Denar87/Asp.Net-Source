    ...
    public partial class AcConfiguration : System.Web.UI.Page
    {
        ...
        protected void dgGridList_RowUpdating(object sender, GridViewUpdateEventArgs e)
        {
            try
            {
                ...
                DropDownList cmbItem = (DropDownList)dgGridList.Rows[e.RowIndex].FindControl("cmbItem");
                string ItemCode = Convert.ToString(cmbItem.SelectedValue);
                string ItemName = Convert.ToString(cmbItem.SelectedItem.Text);
                ...
                int result = objCofg.GetAc_UpdateConfiguration(Id_, OCode, Company_Code, ItemCode, ItemName, LedgerCode, LedgerName, ModuleId_, ModuleName_, TransactionNature_);
